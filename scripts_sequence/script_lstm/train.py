...
def train():
    logger = setup_logger('sequence_training')
    logger.info("Starting Sequence LSTM training")
    ...
    model = SequenceLSTM(CONFIG).to(device)
    ...
    for epoch in range(CONFIG['training']['epochs']):
        ...
        if val_metrics['f1'] > best_f1:
            ...
            torch.save(model.state_dict(), f"{CONFIG['paths']['models']}/sequence_model.pt")
    ...
    model.load_state_dict(torch.load(f"{CONFIG['paths']['models']}/sequence_model.pt"))
    test_metrics = evaluate(model, test_loader, criterion, device)
    
    logger.info("=" * 50)
    logger.info("TEST RESULTS (Sequence LSTM)")
    logger.info("=" * 50)
