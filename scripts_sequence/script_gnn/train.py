...
def train():
    logger = setup_logger('gnn_training')
    ...
    model = GNNFraudDetector(CONFIG).to(device)
    ...
    for epoch in range(CONFIG['training']['epochs']):
        ...
        if val_metrics['f1'] > best_f1:
            ...
            torch.save(model.state_dict(), f"{CONFIG['paths']['models']}/gnn_model.pt")
    ...
    model.load_state_dict(torch.load(f"{CONFIG['paths']['models']}/gnn_model.pt"))
    test_metrics = evaluate(model, test_loader, criterion, device)
    
    logger.info("=" * 50)
    logger.info("TEST RESULTS (GNN Model)")
    logger.info("=" * 50)
